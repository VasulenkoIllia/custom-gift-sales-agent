...
> **Порт БД локально:** у проді Postgres ізольований (без хост-порту). Щоб запускати додаток на хості (`npm run dev` → `localhost:5432`), є `docker-compose.override.yml` (gitignored), який публікує порт лише локально. Він уже створений; за потреби змініть `POSTGRES_PORT` у `.env`.
